...
build/relax/Makefile:
	mkdir -p build/relax
	cd build/relax && cmake -G "Unix Makefiles" -DARM_TOOLCHAIN_DIR=${ARM_GCC_BIN_FOLDER} -DCMAKE_TOOLCHAIN_FILE=$(ARM_TOOLCHAIN_FILE) -DTOP_BIN_DIR:STRING=$(CURRENT_DIR)/build/relax -DBOARD_NAME:STRING=Relax $(CURRENT_DIR)

config-ankle: build/ankle/Makefile
build/ankle/Makefile:
	mkdir -p build/ankle
	cd build/ankle && cmake -G "Unix Makefiles" -DARM_TOOLCHAIN_DIR=${ARM_GCC_BIN_FOLDER} -DCMAKE_TOOLCHAIN_FILE=$(ARM_TOOLCHAIN_FILE) -DTOP_BIN_DIR:STRING=$(CURRENT_DIR)/build/ankle -DBOARD_NAME:STRING=Ankle $(CURRENT_DIR)
